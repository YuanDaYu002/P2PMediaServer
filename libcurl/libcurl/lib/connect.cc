...
#ifdef ENABLE_IPV6
 perror("hle hle not support ipv6!\n ");
  ...
  printf("AF_INET(%d) AF_INET6(%d) AF_UNIX(%d) sa_family = %d\n",AF_INET,AF_INET6,AF_UNIX, sa->sa_family);
  switch(sa->sa_family) {
    case AF_INET:
	
 	printf("hle hle into AF_INET!\n ");
                                ...
                                curl_socket_t *sockp)
{
  ...
  *sockp = CURL_SOCKET_BAD;
  ...
  if(!Curl_getaddressinfo((struct sockaddr*)&addr.sa_addr,
    ...
    failf(data, "hle hle sa_addr inet_ntop() failed with errno %d: %s",
          errno, Curl_strerror(conn, errno));
...
/**hle  修改bug增加的代码段*/
//IPV6不支持
  printf("ai->ai_addr->sa_family(%d)\n",ai->ai_addr->sa_family);
  printf("ai->ai_addr->sa_data(%s)\n",ai->ai_addr->sa_data);
  if(ai->ai_addr->sa_family != AF_INET||\
     ai->ai_addr->sa_family !=AF_INET6||\
     ai->ai_addr->sa_family !=AF_UNIX)
     {
        while(NULL == hle_ip)//等待IP解析成功
          usleep(1000);
        ...
        mysock.sin_family = AF_INET;  //设置地址家族IPV4
        if(NULL != hle_port)
        {
            int port = atoi(hle_port);
            mysock.sin_port = htons(port);  //设置端口
        }
          
        mysock.sin_addr.s_addr = inet_addr(hle_ip);  //设置地址

        memcpy(ai->ai_addr,&mysock,sizeof(mysock));
...
/***************************/
  ...
  printf("addr->sa_addr.sa_family(%d)\n",addr->sa_addr.sa_family);
  printf("addr->sa_addr.sa_data(%s)\n",addr->sa_addr.sa_data);
