...
void * P2P_client_task_func(void*P2P_handle)
{
	...
	while(1)//条件：客户端没退出
	{
		...
		if(ret < 0)//读取出错
		{
			...
			{
				ERROR_LOG("fail to read Test Mode!!\n");
				
			}	
			else if (ERROR_PPCS_SESSION_CLOSED_REMOTE == ret) 
			{
				DEBUG_LOG("Remote site call close!!\n");
				
			}
			else 
			{
				DEBUG_LOG("PPCS_Read: Channel=%d, ret=%d\n", CH_CMD, ret);
		...
		//信令头读取成功！对信令进行解析
		med_ser_signal_parse(SessionID,(void*)&cmd_header,ReadSize);
