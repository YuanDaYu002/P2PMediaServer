...
static void set_h264_avbr_attr(VENC_CHN_ATTR_S *vencAttr, ENC_STREAM_ATTR *stream_attr, int enc_level)
{
    ...
    h264AVbr->u32SrcFrmRate = hal_get_max_frame_rate();
...
static void set_rcparam(int chn)
{
    ...
    vrp.u32RowQpDelta = 0;
    vrp.s32FirstFrameStartQp = 30;
    VENC_PARAM_H265_AVBR_S* pha = &(vrp.stParamH265AVbr);
    pha->s32ChangePos = 90;
    //pha->u32MinIprop = 1;
    //pha->u32MaxIprop = 100;
    //pha->s32MaxReEncodeTimes = 2;
    //pha->bQpMapEn = HI_FALSE;
    pha->s32MinStillPercent = 60;
    pha->u32MaxStillQP = 40;
    pha->u32MaxQp = 44;
    pha->u32MinQp = 24;
    pha->u32MaxIQp = 40;
    pha->u32MinIQp = 20;
...
#else
    vencAttr->stGopAttr.enGopMode = VENC_GOPMODE_SMARTP;
    vencAttr->stGopAttr.stSmartP.u32BgInterval = 30;
...
static ENC_STREAM_PACK *video_stream_to_pack(int encChn, VENC_STREAM_S *stream)
{
    ...
    if (stream_is_iframe(stream)) {
        ...
        iframe_info->pts_msec = (HLE_U32) (stream->pstPack->u64PTS / 1000);
        HLE_U8 wday;
        ...
        pframe_info->pts_msec = (HLE_U32) (stream->pstPack->u64PTS / 1000);
...
static ENC_STREAM_PACK *audio_stream_to_pack(int encChn, AUDIO_STREAM_S *stream)
{
    ...
    aframe_info->sample_rate = AUDIO_SR_8000;
...
void *enc_get_stream_proc(void *arg)
{
    ...
    if (aenc_fd < 0)
    {
    ERROR_LOG("HI_MPI_AENC_GetFd failed, ret = %#x\n", aenc_fd);
    return NULL;
    }
    FD_SET(aenc_fd, &old_fds);
    if (max_fd < aenc_fd) {
    max_fd = aenc_fd;
    }
    ...
    while (enc_ctx.running) {
        ...
        time_out.tv_sec = 1;
        time_out.tv_usec = 0;
        ret = select(max_fd1, &read_fds, NULL, NULL, &time_out);
        if (ret == -1) {
            if (errno == EINTR) {
                continue;
            }
        ...
        //now we got data to deal with
        for (i = 0; i < ENC_STREAM_NUM; i++) {
            ENC_CHN_CONTEX *chn_ctx = enc_ctx.encChn + i;
            pthread_mutex_lock(&chn_ctx->lock);
            if (chn_ctx->state == ENC_STATE_RUNING && FD_ISSET(venc_fd[i], &read_fds)) {
