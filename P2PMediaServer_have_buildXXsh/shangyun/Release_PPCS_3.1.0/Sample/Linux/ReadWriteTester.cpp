...
{	
	...
	char * buf = (char*)malloc(sizeof(STRUCT_GET_MAC_REQUEST));
	cmd_header_t* mycmd =(cmd_header_t*) buf;
	mycmd->command = CMD_GET_MAC;
	mycmd->head = HLE_MAGIC;
	mycmd->type = 1;
	mycmd->length = 0;
	
	
	ret = PPCS_Write(gSessionID, CH_CMD, (char*)mycmd, sizeof(STRUCT_GET_MAC_REQUEST)+mycmd->length);
