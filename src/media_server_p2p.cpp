...
int p2p_conect(p2p_handle_t *P2P_handle)
{
	...
	if(P2P_handle->WakeupKey) //支持唤醒的逻辑部分（Advanced P2P）
	{
		...
		st_debug("DID[%s] %u Byte --encryption--> CMD[%s] %u Byte\n", DID, (unsigned)strlen(DID), CMD, SizeOfCMD);
...
int p2p_listen(p2p_handle_t *P2P_handle)
{
	if( P2P_status != sleeping)
	{
		...
		{
			printf("into CreateThread_LoginStatus_Check\n");
		...
		if(P2P_handle->Session_num < MAX_CLIENT_NUM)//同时登录的客户端数量不能超过限制
		{
			...
			{
				printf("ERROR, Call_P2P_Listen error! gSessionID = %d\n ",SessionID);
				return SessionID;
			}
		...
		{
			printf("ERROR, %s %s line %d :too many client connect!\n ",__FILE__,__func__,__LINE__);
			return -1;
		}
	...
	{ 
		printf("ERROR, %s %s line %d :Call_P2P_Listen error!! P2P_status is sleeping!\n ",__FILE__,__func__,__LINE__);
		return -1; 
	}
...
void * P2P_client_task_func(void*P2P_handle)
{
	...
	int ret = PPCS_Read(handle->SessionID, CH_CMD, (CHAR*)&Mode, &ReadSize, 2000);
	if(ret < 0)
	{
