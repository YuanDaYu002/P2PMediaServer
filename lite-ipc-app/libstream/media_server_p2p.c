...
void * P2P_client_task_func(void*P2P_handle)
{
	...
	HLE_S32 ret = add_one_session_to_arr(SessionID);
	if(ret < 0)
	{
	...
	while(ERROR_PPCS_SUCCESSFUL == PPCS_Check(SessionID,&Sinfo))//条件：客户端没退出
	{
		memset(&Sinfo,0,sizeof(st_PPCS_Session));
		memset(&cmd_header,0,sizeof(cmd_header_t));
		//读取信令头
		ReadSize = sizeof(cmd_header_t);
		printf("PPCS_Read CMD header ReadSize(%d)\n",ReadSize);
		//PPCS_Read 每次返回后，ReadSize 会被填入实际读到的字节数。
		ret = PPCS_Read(SessionID, CH_CMD, (CHAR*)&cmd_header, &ReadSize, 2000);
		if(ret < 0)//读取出错
		{
			if (ERROR_PPCS_TIME_OUT == ret) 
			{
				ERROR_LOG("fail to read CMD Header!!\n");
		...
		printf("PPCS_Read CMD Header success! SessionID(%d) read(%d)bytes!\n",SessionID,ReadSize);
	...
	del_one_session_to_arr(SessionID);
	PPCS_Close(SessionID);//关闭Session的时候注意将 handle->Session_num --
	//操作公共资源时注意互斥情况
