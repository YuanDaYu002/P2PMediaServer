...
HLE_S32 get_one_encoded_frame(HLE_S32 stream_id,HLE_S8 have_audio,const void **pack_addr,const void**frame_addr,HLE_S32* frame_length)
{
...
//debug
#if 0
	printf("get one frame before inc pack ref ");
	spm_pack_print_ref(pack);
	/*注意：获取帧发送完成后要记得将帧（包）的引用计数 -1 ，不然会导致缓存队列的包无法释放*/
	spm_inc_pack_ref(pack);

	//debug
	//在get的时候加了一次索引
	printf("get one frame after inc pack ref ");
	spm_pack_print_ref(pack);
#endif
...
HLE_S32 get_one_encoded_frame(HLE_S32 stream_id,HLE_S8 have_audio,const void **pack_addr,const void**frame_addr,HLE_S32* frame_length)
{
	...
	spm_inc_pack_ref(pack);
	*pack_addr = pack;
	*frame_addr = pack->data + skip_len;
	*frame_length = pack->length - skip_len;
	printf("get pack pack_addr(%p)  frame_addr(%p)  frame_length(%d) bytes\n",*pack_addr,*frame_addr,*frame_length);
#endif
...
//	DEBUG_LOG("dec_frame_ref free pack addr = %p\n\n",pack_addr);
	//encoder_release_packet((ENC_STREAM_PACK *)pack);
...
#if 1
extern  int connected_client_num;
int app_main(int argc, char *argv[])
{
   // int ret = hal_init(200, VIDEO_STD_NTSC);
	int ret = hal_init(300, VIDEO_STD_NTSC);
    if (HLE_RET_OK != ret) {
