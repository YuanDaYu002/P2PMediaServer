...
HLE_S32 motion_detect_val = 0; //初始化

typedef struct {
	//pthread_mutex_t lock;
	HLE_S32 motion_chn_ison;
	HLE_S32 usr_config_level;
...
int motion_detect_data_proc(MOTION_CONTEX* ctx)
{
	...
	for (i = 1; i < blob->u8RegionNum; ++i) {
		area += blob->astRegion[i].u32Area; //printf("{%u;;[%d].%u }\n", area,i, blob->astRegion[i].u32Area);
	}

	HLE_S32 lvl = area * 100 / (mdSize.u32Width * mdSize.u32Height);
	if (lvl >= sensitive_level[ctx->usr_config_level]) {
...
void *motion_detect_proc(void *arg)
{
	...
	while (vda_md_proc_flag) {
		...
		if (!first) {
			...
			if (HLE_RET_OK == ret) {
				ret = HI_IVS_MD_Process(ctx->chn, &ctx->img[idx], &ctx->img[1 - idx], &sad, &ctx->blob);
				if (HI_SUCCESS == ret) {
...
int motion_detect_init(void)
{
	...
	MD_ATTR_S mdAttr;
	mdAttr.enAlgMode = MD_ALG_MODE_BG;
	mdAttr.enSadMode = IVE_SAD_MODE_MB_4X4;
	mdAttr.enSadOutCtrl = IVE_SAD_OUT_CTRL_THRESH;
	mdAttr.u16SadThr = 200;
	mdAttr.u16Width = mdSize.u32Width;
	mdAttr.u16Height = mdSize.u32Height;
	mdAttr.stAddCtrl.u0q16X = 32768;
	mdAttr.stAddCtrl.u0q16Y = 32768;
...
#define MOTION_TETECT_CONFIG "/configs/motion_detect"
...
int motion_detect_arg_init(void)
{
	FILE *index_fp = NULL;
	int ret;
	if (access(MOTION_TETECT_CONFIG, F_OK)) {
